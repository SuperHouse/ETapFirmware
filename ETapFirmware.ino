 ...
 * This version operates differently to the Ryemetal Hotshot V3 electronic tap:
 *  - Sequence starts at short warm, which is the most commonly desired result
 *  - Sequence dwells on short warm for longer, so selecting another mode requires
 *     waiting a longer time. This makes it harder for guests who are unfamiliar with
 *     the tap to get into a different state they weren't expecting, particularly hot.
 *  - No delay on initial hand arrival: goes immediately to short warm.
 */
...
long stateInterval         = 1000;  // Interval to pause between changing states
long stateInterval2        = 5000;  // A special long pause to dwell longer in the first state
...
void setup() {
  Serial.begin(9600);
  Serial.println("eTap starting up in native mode");
...
void loop() {
  ...
  /*
   * Order is:
   *  Off -> short warm -> short cold -> short hot -> long warm -> long cold -> long hot
   */
  switch( currentState )
  {
     case STATE_OFF:
       ...
       digitalWrite(WARM_VALVE, LOW);
       digitalWrite(HOT_VALVE,  LOW);
       if( handPresent )   // No delay here, so immediately proceed to next state
       {
     ...
     case STATE_SHORT_WARM:
       ...
       if( handPresent && currentStateDuration > stateInterval2 )
       {
